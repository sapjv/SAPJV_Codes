...
Then, we can know that U is an articulation point if the following condition is satisfied: discovery_time[U] <= low[V] ( V in this case 
represents an adjacent node of U ). To implement this, in each node V we will store some identifier of its corresponding node low found, 
this identifier will be the corresponding low's discovery time because it helps us to know when the node low was discovered, hence it 
helps us to know by which node we can discover U first.

--------------------------------------
 
Applications of Articulation Points ( Cut vertices ) : 

1. Cut vertices ( Articulation Points ) denote the vertices that are critical in keeping the graph connected. Removing the vertex results 
in splitting of the graph.
...
3. Articulation Points represent vulnerabilities in a network – single points whose failure would split the network into 2 or more disconnected components.
